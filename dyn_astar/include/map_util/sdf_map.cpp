...
// F_get_val和F_set_val是两个句柄函数
// https://blog.csdn.net/tugepaopaoo/article/details/130167750
void SDFMap::fillESDF(F_get_val f_get_val, F_set_val f_set_val, int start, int end, int dim) {
  int v[mp_.map_voxel_num_(dim)];
  double z[mp_.map_voxel_num_(dim) + 1];

  int k = start;  // 曲线的id
  v[start] = start; // 第一个曲线的顶点
  z[start] = -std::numeric_limits<double>::max(); // 第1条抛物线的左端点
  z[start + 1] = std::numeric_limits<double>::max();  // 第1条抛物线的右端点


  for (int q = start + 1; q <= end; q++) {  //遍历所有抛物线，每一个顶点都有一个对应的抛物线
    k++;    // 第二条曲线开始
    double s; // 与上一条抛物线的顶点在x轴上面的位置

    do {
      k--;   // k表示上一条曲线，vk表示上一条曲线的顶点，q表示当前抛物线的顶点
      // s就是上一条曲线k和这一条曲线（以q为顶点的）抛物线的交点。
      s = ((f_get_val(q) + q * q) - (f_get_val(v[k]) + v[k] * v[k])) / (2 * q - 2 * v[k]);
    } while (s <= z[k]); //zk也会改变，就是一直递减到在zk的右边就可以了

    k++;  // 就是zk+1就是这个s

    v[k] = q; // vk就是q
    z[k] = s; 
    z[k + 1] = std::numeric_limits<double>::max(); // 继续k+1就是无穷大
  }

  k = start;

  for (int q = start; q <= end; q++) {  // 遍历所有像素点
    while (z[k + 1] < q) k++;  //z[k+1]在q的右侧，那就说明zk在q的左侧，vk是顶点，
    // 同时再计算距离q离vk的距离，再加上vk离障碍物的距离，就是q的当前距离。
    double val = (q - v[k]) * (q - v[k]) + f_get_val(v[k]);
    f_set_val(q, val); // 存放一个偏置量给下一个维度使用
  }
}
...
void SDFMap::updateESDF3d() {
  ...
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
    for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
      fillESDF(
          [&](int z) {
            // the same to toAddress
            return md_.occupancy_buffer_neg[x * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + y * mp_.map_voxel_num_(2) + z] == 1 ? 0 :
                std::numeric_limits<double>::max();
          },
          [&](int z, double val) { md_.tmp_buffer1_[toAddress(x, y, z)] = val; }, min_esdf[2],
          max_esdf[2], 2);
...
int SDFMap::setCacheOccupancy(Eigen::Vector3d pos, int occ) {
  ...
  // // 加速光线投射的标志缓冲区, 命中+未命中+1
  md_.count_hit_and_miss_[idx_ctns] += 1;

  if (md_.count_hit_and_miss_[idx_ctns] == 1) {
    // 缓存体素队列，如果是第一次，那就放到缓存体素里面
  ...
  if (!mp_.use_depth_filter_) {
    for (int v = 0; v < rows; v++) {
      ...
      for (int u = 0; u < cols; u++) {
        ...
        depth = (*row_ptr++) / mp_.k_depth_scaling_factor_; //深度值除以缩放，得到实际的深度值
        // 得到在相机坐标系下的三维坐标
        // fx 和fy分别是x轴和y轴的焦距
...
Eigen::Vector3d SDFMap::closetPointInMap(const Eigen::Vector3d& pt, const Eigen::Vector3d& camera_pt) {
  Eigen::Vector3d diff = pt - camera_pt;  // 相机指向目标点的向量
  // 相机到地图的最大边界
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  // 在原来的local_map_margin的基础上面，再加一些边缘
  ...
  // int inf_step_z = 1;
  // 某个cube膨胀之后的大小就是pow(2 * inf_step + 1, 3)个方块
