...
struct MappingParameters
{
  ...
  double esdf_slice_height_, visualization_truncate_height_, virtual_ceil_height_,
      ground_height_;
...
{
  return x * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + y * mp_.map_voxel_num_(2) +
         z;
}
...
inline int SDFMap::getOccupancy(Eigen::Vector3i id)
{
  if (id(0) < 0 || id(0) >= mp_.map_voxel_num_(0) || id(1) < 0 ||
      id(1) >= mp_.map_voxel_num_(1) || id(2) < 0 || id(2) >= mp_.map_voxel_num_(2))
    return -1;
